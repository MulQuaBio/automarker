...
def main(filelocation, targetfile,studentspec, modulespec, testspec):
    ...
    try:
        run_result = subprocess.run(f"bash {targetfile}", cwd=codedirpath, shell=True, text=True, timeout = timeout,
                                    stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    except subprocess.TimeoutExpired as e:
        timedout = True
        logger.debug("Timed out as expected (asking for input). No mark loss.")
        run_result = e
